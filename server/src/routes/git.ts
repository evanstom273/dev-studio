...
export function createGitRouter(projects: ProjectService): Router {
	const router = Router()
	...
	router.post('/:projectId/commit', asyncHandler(async (req, res) => {
		...
		const hash = await git.commit(await pathFor(param(req, 'projectId')), message)
		res.json({ hash })
	}))

	router.post('/:projectId/fetch', asyncHandler(async (req, res) => {
		const { remote = 'origin' } = req.body as { remote?: string }
		await git.fetch(await pathFor(param(req, 'projectId')), remote)
		res.json({ ok: true })
	}))

	router.post('/:projectId/pull', asyncHandler(async (req, res) => {
		const { remote = 'origin', branch, rebase = false } = req.body as { remote?: string; branch?: string; rebase?: boolean }
		await git.pull(await pathFor(param(req, 'projectId')), remote, branch, rebase)
		res.json({ ok: true })
	}))

	router.post('/:projectId/push', asyncHandler(async (req, res) => {
		const { remote = 'origin', branch, force = false } = req.body as { remote?: string; branch?: string; force?: boolean }
		await git.push(await pathFor(param(req, 'projectId')), remote, branch, force)
		res.json({ ok: true })
	}))

	router.post('/:projectId/checkout', asyncHandler(async (req, res) => {
		const { name, create = false } = req.body as { name: string; create?: boolean }
		await git.checkout(await pathFor(param(req, 'projectId')), name, create)
		res.json({ ok: true })
	}))
